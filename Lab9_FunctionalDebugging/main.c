...
unsigned long Led;
unsigned long PF0; //negative logic switch
unsigned long PF4; //negative logic switch
...
int main(void){  unsigned long i,last,now;
  ...
  while(1){
		PF4 = GPIO_PORTF_DATA_R&0x10; //Port F Bit/Pin 4 i.e Switch 1
		PF0 = GPIO_PORTF_DATA_R&0x01;	//Port F Bit/Pin 0 i.e Switch 2
		Led = GPIO_PORTF_DATA_R&0x02; //Port F bit/Pin 1 i.e red LED
		if( PF0 == 0x00 || PF4 == 0x00) { //toggles LED aslong as a switch is pressed
			   // read previous
